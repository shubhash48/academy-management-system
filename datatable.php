...
{
$aColumns = array( 's.id','s.sname','s.balance','s.fees','b.course','s.contact','s.joindate');
	...
	{
	$condArr[] = "b.id = '".mysqli_real_escape_string($conn,$_GET['course'])."'";
	...
	$sWhere = " WHERE b.id=s.course and and s.balance>0 ";
	...
	$sQuery = "SELECT COUNT(".$sIndexColumn.") as cc
		FROM   ".$sTable." WHERE b.id=s.course and s.balance>0  ";
...
$aColumns = array( 's.id','s.sname','s.balance','s.fees','b.course','s.contact','s.joindate');
	...
	/* Total data set length */
	$sQuery = "SELECT COUNT(".$sIndexColumn.") as cc
		FROM   ".$sTable." WHERE b.id=s.course   ";
	$rResultTotal = $conn->query( $sQuery ) or die(mysqli_error($conn));
	$aResultTotal = $rResultTotal->fetch_assoc();
	$iTotal = $aResultTotal['cc'];
